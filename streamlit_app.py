...
if feature_option == "4대보험료 검증 시스템":
    ...
    # ✅ 사용자 파일 업로드 (다중 파일 가능)
    uploaded_insurance_files = st.file_uploader(
    ...
    if uploaded_insurance_files:
        try:
            # 📌 임시 폴더 생성
            temp_dir = tempfile.mkdtemp()
            merged_excel_path = os.path.join(temp_dir, "merged_insurance_data.xlsx")

            # 📌 병합을 위한 새로운 워크북 생성
            merged_wb = Workbook()
            merged_wb.remove(merged_wb.active)  # 기본 시트 제거

            # 📌 각 파일의 시트를 유지하면서 병합 (덮어쓰기 방식)
            for uploaded_file in uploaded_insurance_files:
                try:
                    # ✅ 원본 엑셀 파일 로드 (수식 & 서식 유지)
                    file_path = os.path.join(temp_dir, uploaded_file.name)
                    with open(file_path, "wb") as f:
                        f.write(uploaded_file.read())
            ...
            if merged_wb.sheetnames:
                merged_wb.save(merged_excel_path)
                st.success("✅ 파일이 병합되었습니다. 다운로드 가능!")
            else:
                # 📌 데이터가 없을 경우 기본 시트 추가
                ws = merged_wb.create_sheet(title="병합 결과")
                ws.append(["업로드된 파일이 없습니다."])  # 기본 메시지 추가
                merged_wb.save(merged_excel_path)
...
if feature_option == "급여 업무 시스템":
    st.subheader("개발중")
if feature_option == "채용 분석 시스템":
    st.subheader("개발중")
